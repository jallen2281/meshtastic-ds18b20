...
{
    LOG_DEBUG("MQTT::publish called: topic=%s, len=%d, retained=%d", topic, strlen(payload), retained);
    
    if (moduleConfig.mqtt.proxy_to_client_enabled) {
        LOG_DEBUG("Using proxy_to_client");
        ...
        LOG_DEBUG("Connected directly, publishing");
        bool result = pubSub.publish(topic, payload, retained);
        LOG_INFO("pubSub.publish returned: %d", result);
        return result;
    } else {
        LOG_WARN("Not connected (proxy_to_client_enabled=%d, isConnectedDirectly=%d)", moduleConfig.mqtt.proxy_to_client_enabled, isConnectedDirectly());
    }
#endif
    return false;
}
...
void MQTT::publishQueuedMessages()
{
    ...
    defined(NRF52_USE_JSON) // JSON is not supported on nRF52, see issue #2804 ### Fixed by using ArduinoJson ###
        if (moduleConfig.mqtt.json_enabled) {
            ...
            LOG_DEBUG("JsonSerialize returned %d bytes", jsonString.length());
            if (jsonString.length() != 0) {
                ...
                LOG_INFO("JSON publish attempt to %s, %u bytes, json_enabled=%d, connected=%d", topicJson.c_str(), jsonString.length(), moduleConfig.mqtt.json_enabled, isConnectedDirectly());
                bool pubResult = publish(topicJson.c_str(), jsonString.c_str(), false);
                LOG_INFO("JSON publish result: %d", pubResult);
            } else {
                LOG_WARN("JsonSerialize returned 0 bytes for packet type %d", env->packet->decoded.portnum);
            }
        } else {
            LOG_DEBUG("JSON publishing disabled (json_enabled=%d)", moduleConfig.mqtt.json_enabled);
        }
#endif // ARCH_NRF52 NRF52_USE_JSON
...
{
    // Log all packets to track what goes to MQTT
    if (mp_decoded.which_payload_variant == meshtastic_MeshPacket_decoded_tag) {
        LOG_INFO("MQTT::onSend - portnum=%d, from=0x%x, to=0x%x", mp_decoded.decoded.portnum, mp_decoded.from, mp_decoded.to);
    } else {
        LOG_INFO("MQTT::onSend - encrypted or unknown variant");
    }
    ...
    if (mp_decoded.which_payload_variant == meshtastic_MeshPacket_decoded_tag) {
        ...
        if (!isFromUs(&mp_decoded) && !isMqttServerAddressPrivate && dontUplink &&
            ...
            LOG_INFO("MQTT onSend - Not forwarding packet due to DontMqttMeBro flag, portnum=%d", mp_decoded.decoded.portnum);
            return;
        }
    ...
    if ((ch.settings.uplink_enabled && !isPKIEncrypted) || isPKIEncrypted) {
        LOG_INFO("MQTT onSend - Will process packet, portnum=%d", mp_decoded.decoded.portnum);
    ...
    defined(NRF52_USE_JSON) // JSON is not supported on nRF52, see issue #2804 ### Fixed by using ArduinoJson ###
            if (moduleConfig.mqtt.json_enabled) {
                ...
                if (jsonString.length() != 0) {
                    ...
                    LOG_INFO("JSON publish message to %s, %u bytes", topicJson.c_str(), jsonString.length());
                    bool pubResult = publish(topicJson.c_str(), jsonString.c_str(), false);
                    LOG_INFO("JSON publish result: %d (topic=%s)", pubResult, topicJson.c_str());
                } else {
                    LOG_WARN("JsonSerialize returned 0 bytes for telemetry packet");
                }
            }
#endif // ARCH_NRF52 NRF52_USE_JSON
        } else {
