        ...
        case meshtastic_PortNum_TELEMETRY_APP: {
            ...
            if (pb_decode_from_bytes(mp->decoded.payload.bytes, mp->decoded.payload.size, &meshtastic_Telemetry_msg, &scratch)) {
                ...
                if (decoded->which_variant == meshtastic_Telemetry_device_metrics_tag) {
                    ...
                    const bool useFahrenheit = moduleConfig.telemetry.environment_display_fahrenheit;
                    auto tempForMqtt = [useFahrenheit](float celsius) {
                        return useFahrenheit ? ((celsius * 9.0f / 5.0f) + 32.0f) : celsius;
                    };
                    auto roundTo3 = [](float value) {
                        int scaled = static_cast<int>((value * 1000.0f) + (value >= 0.0f ? 0.5f : -0.5f));
                        return scaled / 1000.0f;
                    };

                    jsonObj["payload"]["temperature"] = roundTo3(tempForMqtt(decoded->variant.environment_metrics.temperature));
                    ...
                    // Add individual temperature readings if available
                    if (decoded->variant.environment_metrics.has_temperature1)
                        jsonObj["payload"]["temperature1"] = roundTo3(tempForMqtt(decoded->variant.environment_metrics.temperature1));
                    if (decoded->variant.environment_metrics.has_temperature2)
                        jsonObj["payload"]["temperature2"] = roundTo3(tempForMqtt(decoded->variant.environment_metrics.temperature2));
                    if (decoded->variant.environment_metrics.has_temperature3)
                        jsonObj["payload"]["temperature3"] = roundTo3(tempForMqtt(decoded->variant.environment_metrics.temperature3));
                    if (decoded->variant.environment_metrics.has_temperature4)
                        jsonObj["payload"]["temperature4"] = roundTo3(tempForMqtt(decoded->variant.environment_metrics.temperature4));
                    if (decoded->variant.environment_metrics.has_temperature5)
                    ...
                    LOG_WARN("Unknown telemetry variant: %d", decoded->which_variant);
                }
            } else if (shouldLog) {
